...
def detect_label_types_from_matches(matches) -> List[str]:
    ...
    if not non_jelly.empty:
        try:
            ...
            elif lt_md.strip() == 'pet' or 'pet food' in lt_md or 'pet_food' in lt_md:
                types.append('pet')
            elif status == 'food':
                types.append('food')
            elif status == 'caution':
...
def smart_print(req: PrintRequest):
    ...
    # ── 主標(health_food / pet / insects / food / caution)
    food_single = ""
    insects_single = ""
    health_food_single = ""
    pet_single = ""
    ...
    if not main_records.empty:
        try:
            ...
            elif lt_str.strip() == 'pet' or 'pet food' in lt_str or 'pet_food' in lt_str:
                pet_single = create_pet_food_label_html(md, 1, FONT_PLACEHOLDER)
                used_status = 'pet'
    ...
    if food_single and jelly_single:
        ...
        html = _multiply_label(pet_single, req.qty)
        msg = f"pet × {req.qty}"
    elif insects_single:
